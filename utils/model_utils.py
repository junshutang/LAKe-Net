...
def mirror_point_cloud_z(batch_data):
    ...
    rotation_matrix = torch.Tensor([[1, 0, 0],
                                    [0, 1, 0],
                                    [0, 0, -1]]).repeat(B,1,1).cuda()
    # print(rotation_matrix.shape, batch_data.shape)
    rotated_data = batch_data.bmm(rotation_matrix)
    return rotated_data
...
def calc_cd(output, gt, calc_f1=False):
    ...
    cd_p = (torch.sqrt(dist1+1e-14).mean(1) + torch.sqrt(dist2+1e-14).mean(1)) / 2
...
def point_match(kp1, kp2, eps=0.005, iterations=50):
    emd_loss = emd.emdModule()
    _, assignment = emd_loss(kp1, kp2, eps, iterations)
    return assignment


def neighbor_match_cd(kp1, kp2, gt, num, eps=0.005, iterations=50):
    # calculate neighbor
    B = kp1.size()[0]
    neighbor1_idx = DF.knn_with_batch(kp1, gt, num, is_max=False).reshape(B, -1)
    neighbor2_idx = DF.knn_with_batch(kp2, gt, num, is_max=False).reshape(B, -1)
    neighbor1 = gt[torch.arange(B).unsqueeze(-1), neighbor1_idx]
    neighbor2 = gt[torch.arange(B).unsqueeze(-1), neighbor2_idx]

    # calculate match neighbor
    kp1_match_idx = point_match(kp1, kp2)  # idx in kp2 best match kp1
    kp_match = kp2[torch.arange(B).unsqueeze(-1), kp1_match_idx.long()]  # (B, kp, 3)
    match_neighbor_idx = DF.knn_with_batch(kp_match, gt, num, is_max=False).reshape(B, -1)
    match_neighbor = gt[torch.arange(B).unsqueeze(-1), match_neighbor_idx]

    # calculate loss
    neighbor1 = neighbor1.reshape(-1, num, 3)
    match_neighbor = match_neighbor.reshape(-1, num, 3)
    neighbor_cd, _ = calc_cd(neighbor1, match_neighbor)
    neighbor_cd = neighbor_cd.reshape(B, -1).mean(-1)

    return neighbor_cd

def neighbor_cd(kp1, gt, num):
    # calculate neighbor
    B = kp1.size()[0]
    neighbor1_idx = DF.knn_with_batch(kp1, gt, num, is_max=False).reshape(B, -1)
    neighbor1 = gt[torch.arange(B).unsqueeze(-1), neighbor1_idx]

    neighbor_cd, _ = calc_cd(neighbor1, gt)
    return neighbor_cd
